...
const CATEGORIES = ['全部', '保養品', '美容耗材', '微整針劑', '藥品', '醫療耗材', '手術耗材', '手術器械', '管制藥', '口服藥', '針劑', '藥膏', '縫線', '電燒', '消毒液', '針具', '美白針',  '保健食品', '消脂針', '客製品', '電音波', '蓓拉瑪雅', '制服', '其它']
    ...
    <Layout profile={profile} onLogout={onLogout} navItems={navItems} activeNav={nav} onNav={setNav}>
      ...
      {modalProduct && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.4)',
          ...
          <div style={{ background:C.white, borderRadius:14, padding:24, width:'100%', maxWidth:380 }}>
      ...
      {nav === 'catalog' && (
        <div>
          <h2 style={{ fontSize:isMobile?17:18, fontWeight:isMobile?500:700, marginBottom:isMobile?12:16, color:C.text }}>
            商品目錄
          </h2>
          ...
          <input
            ...
            placeholder="🔍 搜尋品項名稱..."
            style={{ width:'100%', padding:isMobile?'10px 14px':'9px 14px',
              border:`1px solid ${C.border}`, borderRadius:8,
              fontSize:isMobile?14:13, marginBottom:isMobile?10:14,
          ...
          <div style={{ display:'flex', gap:6,
            ...
            marginBottom:isMobile?14:16, paddingBottom:isMobile?4:0 }}>
            {CATEGORIES.map(cat => (
              <button key={cat} onClick={() => setActiveCat(cat)}
                style={{ padding:isMobile?'6px 14px':'5px 14px', borderRadius:20, border:'1px solid',
                  fontSize:12, cursor:'pointer', flexShrink:isMobile?0:undefined, whiteSpace:'nowrap',
                ...
                <div key={p.id} style={{ background:C.white, border:`1px solid ${C.border}`,
                  ...
                  <div style={{ width:90, flexShrink:0 }}>
                    {p.image_url
                      ...
                      : <div style={{ width:'100%', minHeight:90, background:C.primaryLight,
              ...
              {filtered.map(p => (
                <div key={p.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden' }}>
                  {p.image_url
                    ...
                    : <div style={{ width:'100%', height:130, background:C.primaryLight,
          ...
          {cartLoading ? (
            ...
            <div style={{ textAlign:'center', padding:'60px 0', color:C.textMuted }}>
              ...
              <button onClick={() => setNav('catalog')}
                style={{ background:C.primary, color:C.white, border:'none',
                  padding:isMobile?'10px 22px':'9px 20px', borderRadius:8, fontSize:13, cursor:'pointer' }}>
                前往商品目錄
              </button>
            </div>
          ) : isMobile ? (
        ...
        <div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {myReqs.map((req, idx) => {
              ...
              return (
                <div key={req.id} style={{ background:C.white, border:`1px solid ${C.border}`,
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                    <div>
                      ...
                      <div style={{ fontSize:isMobile?11:12, color:C.textMuted, marginTop:isMobile?3:4 }}>
                        {isMobile ? `${req.store_name}　${req.submit_date}` : `門市：${req.store_name || '-'}`}
                      </div>
                    </div>
                  ...
                  {req.status === 'rejected' && (
                    <button onClick={() => {
                      ...
                      style={{ background:C.blue, color:C.white, border:'none',
                        ...
                        borderRadius:7, fontSize:isMobile?13:12, cursor:'pointer',
