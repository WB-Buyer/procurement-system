...
const C = {
  ...
  red: '#B83232', redLight: '#FDEAEA', blue: '#185FA5',
}
...
export default function StaffApp({ profile, onLogout }) {
  ...
  function openModal(product) {
    ...
    setStockQty(''); setItemNote(''); setReqCount('')
  }
  ...
  const MobileCatalog = () => (
    <div>
      <h2 style={{ fontSize:17, fontWeight:500, marginBottom:12, color:C.text }}>商品目錄</h2>

      {/* 搜尋 */}
      <input value={search} onChange={e => setSearch(e.target.value)}
        placeholder="🔍 搜尋品項名稱..."
        style={{ width:'100%', padding:'10px 14px', border:`1px solid ${C.border}`, borderRadius:8,
          fontSize:14, marginBottom:10, background:C.white, color:C.text, boxSizing:'border-box' }} />

      {/* 類別 pills — 橫向捲動 */}
      <div style={{ display:'flex', gap:6, overflowX:'auto', marginBottom:14, paddingBottom:4 }}>
        {CATEGORIES.map(cat => (
          <button key={cat} onClick={() => setActiveCat(cat)}
            style={{ padding:'6px 14px', borderRadius:20, border:'1px solid', fontSize:12,
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {filtered.map(p => (
          <div key={p.id} style={{ background:C.white, border:`1px solid ${C.border}`,
            ...
            <div style={{ width:90, flexShrink:0 }}>
              {p.image_url
                ...
                : <div style={{ width:'100%', height:'100%', minHeight:90, background:C.primaryLight,
            ...
            <div style={{ padding:'10px 12px', flex:1, display:'flex', flexDirection:'column', justifyContent:'space-between' }}>
              <div>
                <span style={{ background:C.primaryLight, color:C.primaryDark, fontSize:9, fontWeight:500,
                  padding:'2px 7px', borderRadius:20, display:'inline-block', marginBottom:5 }}>
                  {p.category}
                </span>
              ...
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginTop:8 }}>
                ...
                <button onClick={() => openModal(p)}
                  style={{ background:C.primary, color:C.white, border:'none',
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign:'center', color:C.textMuted, padding:'40px 0' }}>找不到符合的品項</div>
        )}
      </div>
    </div>
  )
  ...
  const MobileCart = () => (
    <div>
      <h2 style={{ fontSize:17, fontWeight:500, marginBottom:12, color:C.text }}>購物車</h2>
      {cartLoading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 0', color:C.textMuted }}>
          ...
          <button onClick={() => setNav('catalog')}
            style={{ background:C.primary, color:C.white, border:'none', padding:'10px 22px', borderRadius:8, fontSize:13, cursor:'pointer' }}>
            前往商品目錄
          </button>
        </div>
      ) : (
        <div>
          ...
          <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden', marginBottom:14 }}>
            {cart.map((item, idx) => (
              <div key={idx} style={{ padding:'12px 14px', borderBottom:`1px solid ${C.border}` }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:8 }}>
                  <div style={{ flex:1, paddingRight:8 }}>
                    <div style={{ fontSize:13, fontWeight:500, color:C.text }}>{item.name}</div>
                    <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>
                      單位：{item.unit}　庫存：{item.stockInfo}
                    </div>
                    {item.itemNote && (
                      <div style={{ fontSize:11, color:C.textMuted, marginTop:2 }}>備註：{item.itemNote}</div>
                    )}
                  </div>
                  {/* 刪除 */}
                  <button onClick={() => removeFromCart(idx)}
                    style={{ width:28, height:28, border:'1px solid #F09595', borderRadius:6,
                      background:C.redLight, cursor:'pointer', color:C.red, fontSize:14, flexShrink:0 }}>×</button>
                </div>
                {/* 數量調整 */}
                <div style={{ display:'flex', alignItems:'center', gap:10 }}>
                  <span style={{ fontSize:12, color:C.textMuted }}>請購數量：</span>
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    <button onClick={() => changeQty(idx, -1)}
                      style={{ width:32, height:32, border:`1px solid ${C.border}`, borderRadius:6,
                        background:C.primaryLight, cursor:'pointer', fontSize:16, color:C.text }}>−</button>
                    <span style={{ fontSize:15, minWidth:28, textAlign:'center', fontWeight:500, color:C.text }}>{item.reqQty}</span>
                    <button onClick={() => changeQty(idx, 1)}
                      style={{ width:32, height:32, border:`1px solid ${C.border}`, borderRadius:6,
                        background:C.primaryLight, cursor:'pointer', fontSize:16, color:C.text }}>+</button>
                  </div>
                  <span style={{ fontSize:12, color:C.primaryDark }}>{item.unit}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <div>
              ...
              <input type="date" value={submitDate} onChange={e => setSubmitDate(e.target.value)}
                style={{ width:'100%', padding:'10px 12px', border:`1px solid ${C.border}`,
                  borderRadius:7, fontSize:14, color:C.text, boxSizing:'border-box' }} />
            </div>
            <div>
              <label style={{ fontSize:12, color:C.textMuted, display:'block', marginBottom:6 }}>整單備註（選填）</label>
              <input value={submitNote} onChange={e => setSubmitNote(e.target.value)}
                placeholder="如：急件優先處理"
                style={{ width:'100%', padding:'10px 12px', border:`1px solid ${C.border}`,
                  borderRadius:7, fontSize:14, color:C.text, boxSizing:'border-box' }} />
            </div>
            <button onClick={submitRequisition} disabled={submitting}
              style={{ padding:'13px', background:C.primary, color:C.white, border:'none',
                borderRadius:8, fontSize:15, fontWeight:500, cursor:'pointer' }}>
              {submitting ? '送出中...' : `送出請購單（共 ${cart.length} 項）`}
            </button>
          </div>
        </div>
      )}
  ...
  const MobileMyReqs = () => (
    <div>
      <h2 style={{ fontSize:17, fontWeight:500, marginBottom:12, color:C.text }}>我的請購</h2>
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        {myReqs.length === 0 && (
          <div style={{ color:C.textMuted, textAlign:'center', padding:'40px 0' }}>尚無請購紀錄</div>
        )}
        {myReqs.map((req, idx) => {
          ...
          return (
            <div key={req.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, padding:14 }}>
              {/* 標頭 */}
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:10 }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:500, color:C.text }}>{orderId}</div>
                  <div style={{ fontSize:11, color:C.textMuted, marginTop:3 }}>{req.store_name}　{req.submit_date}</div>
                </div>
                ...
                {req.requisition_items?.map((i, ii) => {
                  ...
                  return (
                    <div key={ii} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                      fontSize:12, color:C.text, padding:'5px 0 5px 10px',
              ...
              {req.status === 'rejected' && req.reject_reason && (
                <div style={{ background:C.redLight, color:C.red, padding:'8px 10px',
              ...
              {req.status === 'rejected' && (
                <button onClick={() => {
                  const items = req.requisition_items?.map(i => ({
                    ...
                    reqQty: i.quantity, stockInfo: `${i.stock_qty} ${i.stock_unit}`,
                    itemNote: i.item_note || '', category: ''
                  })) || []
                  setCart(items); showToast('品項已重新加入購物車'); setNav('cart')
                }}
                  style={{ background:C.blue, color:C.white, border:'none',
                    padding:'8px 16px', borderRadius:7, fontSize:13, cursor:'pointer', width:'100%' }}>
                  一鍵重加購物車
                </button>
              )}
            </div>
  ...
  const DesktopCatalog = () => (
    <div>
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜尋品項名稱..."
        style={{ width:'100%', padding:'9px 14px', border:`1px solid ${C.border}`, borderRadius:8,
          fontSize:13, marginBottom:14, background:C.white, color:C.text }} />
        ...
        {filtered.map(p => (
          <div key={p.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, overflow:'hidden' }}>
            ...
            <div style={{ padding:'12px 14px' }}>
              <span style={{ background:C.primaryLight, color:C.primaryDark, fontSize:10, fontWeight:500,
                padding:'2px 8px', borderRadius:20, display:'inline-block', marginBottom:8 }}>{p.category}</span>
              ...
              <button onClick={() => openModal(p)}
                style={{ width:'100%', background:C.primary, color:C.white, border:'none',
                  padding:'7px', borderRadius:7, fontSize:12, cursor:'pointer' }}>
                加入購物車
              </button>
            </div>
          </div>
            ...
            <div key={req.id} style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:12, padding:16 }}>
              ...
              <div style={{ marginBottom: req.status === 'rejected' ? 10 : 0 }}>
                ...
                {req.requisition_items?.map((i, ii) => {
                  ...
                  return (
                    <div key={ii} style={{ display:'flex', justifyContent:'space-between', alignItems:'center',
                      fontSize:12, color:C.text, padding:'4px 0 4px 12px', borderLeft:`2px solid ${C.border}`, marginBottom:3 }}>
                      <span>{i.products?.name} ×{i.quantity} {i.products?.unit}</span>
                      {owner && (
                        <span style={{ display:'inline-flex', alignItems:'center', gap:4,
                          background:C.primaryLight, color:C.primaryDark,
                          padding:'2px 8px', borderRadius:20, fontSize:10, fontWeight:500, flexShrink:0, marginLeft:8 }}>
                          <span style={{ width:5, height:5, borderRadius:'50%', background:C.primaryDark, display:'inline-block' }}></span>
                          {owner}
                        </span>
                      )}
      ...
      <div style={{ background:C.white, borderRadius:14, padding:24,
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <div style={{ padding:'8px 12px', background:C.primaryLight, borderRadius:7, fontSize:13,
              color:C.primaryDark, display:'flex', alignItems:'center' }}>
              {modalProduct.unit}
            </div>
          </div>
        </div>
        ...
        <div style={{ marginBottom:14 }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            ...
            <div style={{ padding:'8px 12px', background:C.primaryLight, borderRadius:7, fontSize:13,
              color:C.primaryDark, display:'flex', alignItems:'center', flexShrink:0 }}>
        ...
        <div style={{ marginBottom:18 }}>
          ...
          <textarea value={itemNote} onChange={e => setItemNote(e.target.value)}
            ...
            style={{ width:'100%', padding:'8px 10px', border:`1px solid ${C.border}`,
              borderRadius:7, fontSize:13, resize:'vertical', color:C.text, boxSizing:'border-box' }} />
        </div>
        <div style={{ display:'flex', gap:8 }}>
          <button onClick={() => setModalProduct(null)}
            style={{ flex:1, padding:'9px', border:`1px solid ${C.border}`, borderRadius:8,
              background:C.primaryLight, fontSize:13, cursor:'pointer', color:C.text }}>
            取消
          </button>
          <button onClick={confirmAddToCart}
            style={{ flex:1, padding:'9px', background:C.primary, color:C.white,
  ...
  return (
    <Layout profile={profile} onLogout={onLogout} navItems={navItems} activeNav={nav} onNav={setNav}>
      {toast && (
        <div style={{ background:C.text, color:'#fff', padding:'10px 16px',
      ...
      {/* Modal（共用） */}
      {modalProduct && <CartModal />}

      {/* 商品目錄 */}
      {nav === 'catalog' && (isMobile ? <MobileCatalog /> : <DesktopCatalog />)}

      {/* 購物車 */}
      {nav === 'cart' && (isMobile ? <MobileCart /> : <DesktopCart />)}

      {/* 我的請購 */}
      {nav === 'myreqs' && (isMobile ? <MobileMyReqs /> : <DesktopMyReqs />)}
    </Layout>
  )
}
